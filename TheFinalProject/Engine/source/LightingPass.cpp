...
bool LightingPass::InitInputLayout(ID3D11Device* device, std::string vsByteCode)
{
...
bool LightingPass::InitRenderTargetView(ID3D11Device* device, IDXGISwapChain* swapChain)
{
...
bool LightingPass::Initialize(ID3D11Device* device, UINT winWidth, UINT winHeight, IDXGISwapChain* swapChain)
{
	std::string vsByteCode = "";

	if (!InitializeShaders(device, m_vertexShader, "LightingPassVS.cso", m_pixelShader, "LightingPassPS.cso", vsByteCode))
	{
...
void LightingPass::Bind(ID3D11DeviceContext* deviceContext, std::vector<ID3D11ShaderResourceView*>gbuffers)
{
...
void LightingPass::Render(ID3D11DeviceContext* deviceContext)
{
